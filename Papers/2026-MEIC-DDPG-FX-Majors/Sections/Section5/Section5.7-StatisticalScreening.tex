...
The screening produced a result that changes how the whole campaign should be read. \textbf{In every one of the seven pairs, the candidate with the highest return failed the test.} Table~\ref{Tables:BetaTraps} lists the strongest cases. A portfolio built by ranking candidates on return would have selected models averaging roughly $+250\%$, almost all of which is market drift rather than policy.

\input{Tables/BetaTraps}
