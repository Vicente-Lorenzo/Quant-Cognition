...
Fixed in the **generator**, never in the output. `Script/Setup/Enum.py` now emits:
...
- **An `// <auto-generated>` header** naming `Script/Setup/Enum.py` and the `python -m Script.Setup.Enum` command.
...
Renumbering is safe because `Script/Setup/Enum.py` regenerates the C# side. Run `python -m Script.Setup.Enum`, rebuild,
...
`Script/Setup/Research.py` provisions after `setup_auth` for the foreign keys, with indexes on `Run(Status)`,
...
**Done when:** `python -m Script.Setup.Research` provisions cleanly; a console run goes Waiting to Running to
...
### 4.8 Record the observation order with the weights — done 2026-09-17
...
Nothing detects a mismatch — the shapes agree, only the meaning of each input changes. **Done:** every
`DDPGStrategyAPI.save()` writes `Observation.json` beside `DDPG/` (momentum and overlap order · account
features · window · shape) and `load()` refuses weights whose recorded layout differs, naming both orders;
weights without the file load with an `Unverified` warning. Backfilled for the champion and the DDPG golden. Do it before 4.7 retrains, so
...
- **`Script/Install.py` and `Script/Task.py` both define `provision()`** — different modules, different jobs,
...
**`Script/Setup/Enum.py` must emit into both projects** once this starts; `OUTPUT_PATH` is currently a single
