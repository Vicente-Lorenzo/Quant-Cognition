...
- **⚠️ torch env — ROOT CAUSE FOUND + FIXED:** the 12pm `Scripts/environment.bat` (`mamba env update -f Quant.yml --prune`) resolved the **unpinned conda `- pytorch`** (conda-forge ships only a **CPU** `pytorch` on Windows) to `pytorch 2.10.0 cpu_mkl` + `mkl 2025.3` + dual OpenMP, colliding with the pip `torch 2.5.1+cu121` → `fbgemm.dll` WinError 127 + `OMP #15`. **Fix:** moved torch off conda into the pip section of `Quant.yml` pinned to the CUDA index (`--extra-index-url …/cu121` + `torch==2.5.1` / `torchvision==0.20.1` / `torchaudio==2.5.1`) so `--prune` can never re-pull the CPU build; one-time repair = uninstall the mixed installs + clean reinstall the cu121 wheels (set `KMP_DUPLICATE_LIB_OK=TRUE` if the dual-OpenMP `#15` persists).
