...
- **(a) Numpy ring-buffer the shared `SeriesAPI`/`Market` (engine-level, UNIVERSAL — NEXT, higher risk).** Replace `Series._data_` (Polars) with a numpy ring buffer: `last()`/`tail()` → O(1) slices; `Market.update_data` appends scalars instead of `extend` + 7-Series re-init; indicators read the buffer. **2-4× on the per-bar walk for EVERY path — offline, realtime, tick, NNFX** — and fixes the ~60k-Polars-chunk accumulation pathology at 10y-H1. **Touches accuracy-critical math SHARED with RealtimeAPI → must stay bit-identical; validate bit-for-bit against the 6 goldens.** Its own deliberate golden-guarded pass, not a close-out. Also: array-backed bar access at load (`_row_to_bar_` builds BarAPI + 5 TickAPI/bar — ~370k objects at 10y-H1); `Tick.__post_init__` allocates ~7-8 `PriceAPI`/tick — route hot construction through the existing `_ingest_` fast path.
...
- **`_step_`:** `ObservationAPI.encode` → read `Portfolio.Equity` → (if `Training` + prev step) `RewardAPI.reward` + `agent.memorise` + `agent.learn` → `agent.decide(explore=Training)` → `ActionAPI.target` → controller → Action list. `_initialize_` resets agent + observation + reward + transient state per episode.
...
2. ✅ **DONE (2026-06-29)** — **MDP layer** — `ModelStrategyAPI` base + `DDPGStrategyAPI`/`SACStrategyAPI`: observation builder, action→order controller, reward, `memorise`, `training` flag, greedy/eval path; unit-tested with fakes (`Tests/Strategy/test_Model.py`, 20 green). See §6.3b.
