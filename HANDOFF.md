...
- **Database `Dataframe.frame` dtype preservation + FK-aware `reorder` (2026-07-13 audit — deferred, not a priority):** dropping the per-fetch `shrink_dtype` would keep DB-declared dtypes (removing the latent Float64→Float32 price downcast) and skip a full rescan on every result — but it is blocked because `reorder`→`migrate`→`diff` is order-**insensitive** and only works today by `shrink` corrupting inferred dtypes into a fake type-mismatch that forces migrate's rebuild. Making `diff`/`migrate` order-sensitive is unsafe: migrate's rename+recreate rebuild (`Database.py` ~970-981) does not re-point inbound FKs, so it would break FK-referenced tables (e.g. `Scheduler`→`Auth.User`) on boot. Correct sequence: reimplement `reorder` FK-aware first (like the manual Auth physical reorder), then the shrink can be safely removed. Rescan is a perf nicety, not a correctness need. The other 6 Database-audit findings were implemented 2026-07-13 (staged).
