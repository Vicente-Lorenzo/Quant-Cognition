...
### 7.1 Components
- **App server (`Environment.Server` task → `Library/Web/Service/Tray.py`):** runs a **waitress** WSGI server binding the app to `127.0.0.1:8050` (loopback only — never exposed directly). Supervised always-on by the orchestrator (Step 8). `Tray.py` runs the server in-process behind a system-tray menu and falls back to headless `Service/Serve.py` if the tray cannot start; `python -m Library.Web.App` is the dev/`debug` entry point.
- **Tunnel (`Environment.Tunnel` task → `Library/Web/Service/Tunnel.py`):** runs `cloudflared --config ~/.cloudflared/config.yml tunnel run Quant`. The named tunnel **`Quant`** (`config.yml` → `ingress: quantcognition.com → http://127.0.0.1:8050`, fallback `http_status:404`) authenticates with its credentials JSON in `~/.cloudflared`. The Cloudflare dashboard holds the DNS record routing `quantcognition.com` to this tunnel.
...
| Task | Trigger | Runs |
| --- | --- | --- |
| `Quant Scheduler` | At logon | `"<Quant pythonw>" Script\Scheduler.py` (silent tray app) |
...
| **`Environment`** | `0 4 * * *` | `Environment.Cache` (`Script/Cache.py` → cache cleanup) → `Environment.Retention` (`Setup/Retention.py` → prunes logs past the 30-day horizon) → `Environment.Version` (`Setup/Version.py` → reports newer vendored frontend releases) → `Environment.Update` (`Setup/Environment.py` → conda refresh, all `Scheduled`) → `Environment.Tunnel` → `Environment.Server` (both `Service`) | keeps the tunnel + app always-on; daily (and at every boot) it suspends them, cleans the caches, prunes retention, checks versions, refreshes the environment, then relaunches in order |
...
1. **`python -m Setup.Install --boot`** — creates + populates every schema, seeds the administrator, registers the three workflows, and schedules the orchestrator (`Quant Scheduler`) at logon in Windows Task Scheduler (the boot task runs the Quant interpreter on `Script\Scheduler.py`). Drop `--boot` to skip the Task-Scheduler step and register it yourself.
2. **Launch the orchestrator** (reboot/re-logon, or run `python Script\Scheduler.py`). It boot-launches the `Environment` workflow — cache cleanup, environment refresh, then tunnel + web app in strict order.
