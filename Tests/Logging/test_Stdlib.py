...
def test_facade_is_a_standard_logger():
    assert isinstance(LoggingAPI(Class="Std"), logging.Logger)

def test_facade_exposes_standard_logger_attributes():
    log = LoggingAPI(Class="Std")
    assert log.name == "Std"
    assert log.handlers == []
...
def test_standard_percent_formatting(recorder):
    ...
    LoggingAPI(Class="Percent").info("value %s and %d", "text", 7)
...
def test_bridge_captures_third_party_records(recorder):
    recorder.set_level(VerboseLevel.Debug)
    LoggingAPI.install(LoggingAPI(Class="Capture"), hooks=False, level=VerboseLevel.Warning)
...
def test_bridge_maps_standard_levels(recorder):
    recorder.set_level(VerboseLevel.Debug)
    LoggingAPI.install(LoggingAPI(Class="Capture"), hooks=False, level=VerboseLevel.Debug)
...
def test_bridge_respects_its_level(recorder):
    recorder.set_level(VerboseLevel.Debug)
    LoggingAPI.install(LoggingAPI(Class="Capture"), hooks=False, level=VerboseLevel.Error)
...
def test_install_is_idempotent(recorder):
    LoggingAPI.install(LoggingAPI(Class="Capture"), hooks=False)
    LoggingAPI.install(LoggingAPI(Class="Capture"), hooks=False)
...
def test_bridge_never_raises_on_a_broken_record(recorder):
    recorder.set_level(VerboseLevel.Debug)
    bridge = BridgeAPI(LoggingAPI(Class="Capture"), VerboseLevel.Debug)
...
def test_excepthook_logs_and_delegates(recorder, monkeypatch):
    ...
    LoggingAPI.install(LoggingAPI(Class="Capture"), stdlib=False, hooks=True)
    import sys
