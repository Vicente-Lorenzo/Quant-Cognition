...
def test_a_flat_space_is_not_staged():
    assert SpaceAPI.staged(FLAT) is False
    assert len(SpaceAPI.unpack_plan(FLAT)) == 1

def test_a_numbered_space_is_staged():
    assert SpaceAPI.staged(STAGED) is True
    assert len(SpaceAPI.unpack_plan(STAGED)) == 3

def test_a_span_applies_to_every_stage_it_covers():
    plan = SpaceAPI.unpack_stages(STAGED)
...
def test_a_reversed_span_is_rejected():
    with pytest.raises(ValueError):
        SpaceAPI.unpack_stages(Block(MoneyManagement={"4-2": {"RiskPercentage": [1.0]}}))

def test_a_gap_in_the_numbering_is_rejected():
    with pytest.raises(ValueError):
        SpaceAPI.unpack_stages(Block(MoneyManagement={"1": {"RiskPercentage": [1.0]}, "3": {"RiskPercentage": [2.0]}}))

def test_a_section_mixing_stages_with_parameters_is_rejected():
    with pytest.raises(ValueError):
        SpaceAPI.unpack_stages(Block(TechnicalManagement={"1": {"Baseline": [["SMA"]]}, "Filter1": [["TT"]]}))
...
def test_a_flat_section_is_constant_across_every_stage():
    ...
    assert SpaceAPI.staged(mixed) is True
    plan = SpaceAPI.unpack_stages(mixed)
...
def test_inheritance_is_recognized_only_in_its_exact_form(value, expected):
    assert SpaceAPI.inherited(value) == expected

def test_inheritance_pins_an_earlier_winner():
    winners = [{"TechnicalManagement": {"Baseline": ["HMA", 20]}}, {}]
    resolved = SpaceAPI.resolve_inheritance(SpaceAPI.unpack_stages(STAGED)[2], winners, 2)
    assert resolved["TechnicalManagement"]["Baseline"] == ["HMA", 20]
    assert SpaceAPI.build_grid(SpaceAPI.unpack_section(resolved))[0].overrides["TechnicalManagement"]["Baseline"] == ["HMA", 20]

def test_inheritance_from_a_later_stage_is_rejected():
    with pytest.raises(ValueError):
        SpaceAPI.resolve_inheritance({"TechnicalManagement": {"Baseline": ["Result=3"]}}, [{}], 1)

def test_inheritance_from_a_barren_stage_is_rejected():
    with pytest.raises(ValueError):
        SpaceAPI.resolve_inheritance({"TechnicalManagement": {"Baseline": ["Result=1"]}}, [{}], 1)
...
def test_staging_turns_a_product_into_a_sum():
    ...
    assert SpaceAPI.measure_plan(SpaceAPI.unpack_plan(product)) == 12
    assert SpaceAPI.measure_plan(SpaceAPI.unpack_plan(summed)) == 7

def test_a_flat_space_expands_exactly_as_before():
    assert SpaceAPI.expand_parameter([["SMA", "EMA"], [10, 20]]) == [["SMA", 10], ["SMA", 20], ["EMA", 10], ["EMA", 20]]
    assert SpaceAPI.rounds_parameter([["SMA", "EMA"], [10, 20]]) == 1

def test_auto_takes_the_ladder_from_the_indicator():
    assert SpaceAPI.rounds_parameter([["SMA"], ["Auto"]]) == 3
    assert SpaceAPI.expand_parameter([["SMA"], ["Auto"]])[:3] == [["SMA", 5], ["SMA", 10], ["SMA", 15]]

def test_a_later_round_refines_around_the_winner():
    assert SpaceAPI.expand_parameter([["SMA"], ["Auto"]], ["SMA", 20], 1) == [["SMA", value] for value in range(12, 29, 2)]
    assert SpaceAPI.expand_parameter([["SMA"], ["Auto"]], ["SMA", 20], 2) == [["SMA", value] for value in range(17, 24)]

def test_a_later_round_pins_the_categorical_slots():
    assert SpaceAPI.expand_parameter([["SMA", "EMA"], ["Auto"]], ["EMA", 20], 2) == [["EMA", value] for value in range(17, 24)]

def test_an_inline_range_needs_no_indicator():
    assert SpaceAPI.expand_parameter([["ATR"], ["10-30:10"]]) == [["ATR", 10], ["ATR", 20], ["ATR", 30]]
    assert SpaceAPI.expand_parameter([[1.0], ["0.5..2.0:0.5"]])[0] == [1.0, 0.5]

def test_admits_prunes_an_impossible_cross():
    grid = SpaceAPI.expand_parameter([["SMAC"], ["Auto"], ["Auto"]])
    assert grid
    assert all(fast < slow for _, fast, slow in grid)

def test_coarse_to_fine_costs_less_than_an_exhaustive_sweep():
    coarse = len(SpaceAPI.expand_parameter([["SMA"], ["Auto"]]))
    refined = len(SpaceAPI.expand_parameter([["SMA"], ["Auto"]], ["SMA", 20], 1)) + len(SpaceAPI.expand_parameter([["SMA"], ["Auto"]], ["SMA", 20], 2))
...
def test_a_range_in_the_leading_slot_expands():
    options = SpaceAPI.unpack_section({'MoneyManagement': {'RiskPercentage': ['0.5..3.0:0.5']}})['MoneyManagement']['RiskPercentage']
    assert [value[0] for value in SpaceAPI.expand_parameter(options)] == [0.5, 1.0, 1.5, 2.0, 2.5, 3.0]

def test_a_range_in_the_leading_slot_refines():
    options = SpaceAPI.unpack_section({'MoneyManagement': {'RiskPercentage': ['0.5..3.0:0.5']}})['MoneyManagement']['RiskPercentage']
    assert SpaceAPI.rounds_parameter(options) == 3
    assert [value[0] for value in SpaceAPI.expand_parameter(options, [1.5], 1)] == [1.0, 1.25, 1.5, 1.75, 2.0]

def test_a_plain_list_in_the_leading_slot_is_untouched():
    options = SpaceAPI.unpack_section({'MoneyManagement': {'RiskPercentage': [[0.5, 1.0, 1.5]]}})['MoneyManagement']['RiskPercentage']
    assert SpaceAPI.rounds_parameter(options) == 1
    assert [value[0] for value in SpaceAPI.expand_parameter(options)] == [0.5, 1.0, 1.5]

def test_auto_without_a_ladder_is_refused_not_taken_literally():
    options = SpaceAPI.unpack_section({'MoneyManagement': {'RiskPercentage': ['Auto']}})['MoneyManagement']['RiskPercentage']
    with pytest.raises(ValueError, match="leading slot is Auto"):
        SpaceAPI.expand_parameter(options)

def test_auto_on_a_head_that_declares_no_ladder_is_refused():
    with pytest.raises(ValueError, match="declares no ladder"):
        SpaceAPI.expand_parameter([["RiskPercentage"], ["Auto"]])

def test_the_refusal_names_the_offending_parameter():
    with pytest.raises(ValueError, match=r"MoneyManagement\.RiskPercentage"):
        SpaceAPI.build_grid({"MoneyManagement": {"RiskPercentage": [["Auto"]]}})

def test_auto_still_expands_wherever_a_ladder_exists():
    assert len(SpaceAPI.expand_parameter([["SMA"], ["Auto"]])) == 20
    assert len(SpaceAPI.expand_parameter([["SMA", "EMA"], ["Auto"]])) == 40