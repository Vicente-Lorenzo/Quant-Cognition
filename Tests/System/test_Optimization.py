...
def test_argmax_takes_the_peak_even_when_isolated():
    ...
    chosen, score = select([(c, scores[c.index]) for c in grid], SelectionMode.Best, adjacency=SpaceAPI.neighborhoods)
    assert chosen.index == 1 and score == 9.0

def test_plateau_prefers_a_stable_region_over_a_lone_spike():
    grid = SpaceAPI.build_grid(SPACE)
    scores = {0: 1.0, 1: 9.0, 2: 1.0, 3: 1.0, 4: 4.0, 5: 4.2, 6: 4.1, 7: 3.9}
    chosen, _ = select([(c, scores[c.index]) for c in grid], SelectionMode.Plateau, adjacency=SpaceAPI.neighborhoods)
    assert chosen.index == 5

def test_selection_accepts_the_mode_by_name():
    grid = SpaceAPI.build_grid(SPACE)
    scored = [(c, float(c.index)) for c in grid]
    assert select(scored, "Best", adjacency=SpaceAPI.neighborhoods)[0].index == 7

def test_selection_ignores_candidates_without_a_score():
    grid = SpaceAPI.build_grid(SPACE)
    scored = [(grid[0], None), (grid[1], 2.0), (grid[2], None)]
    assert select(scored, SelectionMode.Best, adjacency=SpaceAPI.neighborhoods)[0].index == 1

def test_selection_without_any_score_returns_nothing():
    grid = SpaceAPI.build_grid(SPACE)
    assert select([(candidate, None) for candidate in grid], SelectionMode.Best, adjacency=SpaceAPI.neighborhoods) is None
...
def test_unpack_space_ignores_absent_and_empty_sections():
    ...
    space = SpaceAPI.unpack_section(SpaceAPI.flatten_space(holder))
...
def test_unpack_space_wraps_a_bare_slot_into_options():
    ...
    space = SpaceAPI.unpack_section(SpaceAPI.flatten_space(Holder(Block({"ATR": ["ATR", [14, 21]]}))))
...
def test_apply_candidate_overrides_only_named_slots():
    applied = SpaceAPI.apply_candidate(_base_(), SpaceAPI.build_grid(SPACE)[0])
...
def test_apply_candidate_never_mutates_the_base():
    base = _base_()
    SpaceAPI.apply_candidate(base, SpaceAPI.build_grid(SPACE)[3])
...
def test_apply_candidate_ignores_parameters_absent_from_the_base():
    ...
    assert "Unknown" not in SpaceAPI.apply_candidate(_base_(), candidate).data["MoneyManagement"]

def test_neighborhoods_match_pairwise_distance():
    grid = SpaceAPI.build_grid(SPACE)
    adjacency = SpaceAPI.neighborhoods(grid)
...
def test_plateau_stays_linear_on_a_large_grid():
    ...
    assert select(scored, SelectionMode.Plateau, adjacency=SpaceAPI.neighborhoods) is not None
