...
def clean(vault):
    yield
    for row in sorted(vault._select_(), key=lambda row: row.get("Parent") is None): vault.delete(row["UID"], by=ADMIN)

def cli(capsys, user: str, *arguments) -> tuple:
    code = Main.CredentialCommandAPI().main(["--database", DATABASE, "--user", user, *arguments])
...
def test_transfer_and_delete(vault, clean, capsys, monkeypatch):
    ...
    assert code == 0 and vault._select_(Name="Mine")[0]["Owner"] == VIEWER
...
def test_a_missing_target_is_rejected(vault, clean, capsys):
...
def test_no_user_acts_as_the_administrator(vault, clean, capsys, monkeypatch):
    ...
    assert code == 0 and administrator is not None and vault._select_(Name="Defaulted")[0]["Owner"] == administrator