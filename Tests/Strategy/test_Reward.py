...
def test_differential_sharpe_finite_and_rewards_gains():
    ...
    values = [reward.reward(equities[i], equities[i - 1]) for i in range(1, len(equities))]
...
def test_reset_clears_state():
    ...
    for equity, previous in ((10100.0, 10000.0), (10200.0, 10100.0)):
        reward.reward(equity, previous)
    reward.reset()
    assert reward._mean_ == 0.0 and reward._square_ == 0.0 and reward._downside_ == 0.0