...
- **`Library/Strategy`**: Strategy framework (`StrategyAPI`, `StrategyType`). Strategy implementations split into `Rule/` (e.g., `Download`, `NNFX`), `Model/` (`DDPG`, `ExtendedDDPG`, `SAC`, `TD3`), and `Hybrid/` (`HybridDDPG`, `HybridExtendedDDPG`, `HybridSAC`, `HybridTD3` — model direction signals managed by the NNFX money/risk machines) subpackages.
