...
- **`Library/Auth`**: Server-side authentication and role-based authorization backed by a Postgres `Auth` schema. `RoleAPI` (ordered `Public` < `Viewer` < `Member` < `Moderator` < `Administrator` with `grants`; the account-menu person icon is tinted per role — Public grey · Viewer cyan · Member green · Moderator blue · Administrator red), `PasswordAPI` (Argon2id hash/verify/rehash), `UserAPI(DatapointAPI)` (the `Auth.User` table — `UID`/`Email`/`Name`/`PasswordHash`/`Role`/`Provider`/`IsActive`/`CreatedAt`/`LastLogin`), `IdentityAPI`/`AnonymousIdentityAPI` (Flask-Login user objects), `AuthProviderAPI` seam with `LocalAuthProviderAPI` (Argon2+DB), `CloudflareAuthProviderAPI` (Cloudflare Access edge-JWT/email header → JIT provision) and `OIDCAuthProviderAPI` (generic OIDC JWT), and `AuthAPI` (the service: installs Flask-Login + `ProxyFix` + hardened cookies on the Dash Flask server, `find`/`locate`/`authenticate`/`login`/`logout`/`provision`/`create`/`touch`/`password`). DB authorization (roles) is decoupled from authentication (local or SSO); SSO users are JIT-provisioned by email as `Viewer`. Cookie `Secure` defaults on; set `QUANT_INSECURE=1` for local http dev. Schema/table + admin seeding via `Setup/Auth.py` (admin password from `QUANT_ADMIN_PASSWORD` or generated).
