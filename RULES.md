...
2. **General:** No docstrings or comments. Maintain tidy files (no trailing spaces). One blank line after `class` and between methods. (Exception: the DRL method modules `Library/Model/Method/DDPG` and `Library/Model/Method/SAC` carry paper-mapping docstrings and comments by explicit request, to enable validation against the source papers — do not strip them.)
