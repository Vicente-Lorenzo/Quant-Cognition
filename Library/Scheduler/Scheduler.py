...
from Library.Utility.Datetime import local_to_utc, utc_now, utc_to_local
...
class SchedulerAPI:
    ...
    @staticmethod
    def _next_(schedule: str, moment: datetime, zone: Union[str, None, Missing] = MISSING) -> datetime:
        cron = croniter(schedule, utc_to_local(moment, zone))
        while (candidate := local_to_utc(cron.get_next(datetime), zone)) <= moment: continue
        return candidate

    @staticmethod
    def _previous_(schedule: str, moment: datetime, zone: Union[str, None, Missing] = MISSING) -> datetime:
        local = croniter(schedule, utc_to_local(moment, zone)).get_prev(datetime)
        return max(candidate for candidate in (local_to_utc(local, zone), local_to_utc(local.replace(fold=1), zone)) if candidate <= moment)

    @staticmethod
    def _due_(schedule: Union[str, None], last: Union[datetime, None], now: datetime, zone: Union[str, None, Missing] = MISSING) -> bool:
        if not schedule: return False
        if last is None: return True
        return SchedulerAPI._next_(schedule, last, zone) <= now

    @staticmethod
    def _timely_(schedule: Union[str, None], opened: Union[datetime, None], now: datetime, zone: Union[str, None, Missing] = MISSING) -> bool:
        if not schedule: return True
        if opened is None: return False
        return SchedulerAPI._next_(schedule, opened - timedelta(seconds=1), zone) <= now
    ...
    def _tasks_(self, db: PostgresDatabaseAPI) -> list[dict]:
        return db.records(schema=TaskAPI.Schema, table=TaskAPI.Table, condition='"Enabled" = :enabled:', parameters={"enabled": True})

    def _latest_(self, db: PostgresDatabaseAPI, tid: str) -> Union[dict, None]:
        return db.first(schema=RunAPI.Schema, table=RunAPI.Table, condition='"TID" = :tid:', order='"StartedAt" DESC NULLS LAST', parameters={"tid": tid})
    ...
    def _is_latest_(self, db: PostgresDatabaseAPI, row: dict) -> bool:
        ...
        return db.first(schema=RunAPI.Schema, table=RunAPI.Table, columns=["UID"], condition=f'{condition} AND "StartedAt" > :started:', parameters={**(parameters or {}), "started": row["StartedAt"]}) is None

    def _retry_(self, db: PostgresDatabaseAPI, now: datetime, budget: int) -> int:
        for row in db.records(schema=RunAPI.Schema, table=RunAPI.Table, condition='"Status" = :retrying:', parameters={"retrying": RunStatus.Retrying.name}):
            if budget <= 0: break
            ...
            if not self._issue_((row["CID"], row["TID"], retry)): continue
    ...
    def _schedule_(self, db: PostgresDatabaseAPI, tasks: list[dict], now: datetime, budget: int) -> int:
        for task in tasks:
            ...
            if not self._issue_((None, task["UID"], 0)): continue
    ...
    def _enabled_workflows_(self, db: PostgresDatabaseAPI) -> list[dict]:
        return db.records(schema=WorkflowAPI.Schema, table=WorkflowAPI.Table, condition='"Enabled" = :enabled:', parameters={"enabled": True})

    def _cycle_(self, db: PostgresDatabaseAPI, wid: str) -> Union[dict, None]:
        return db.first(schema=CycleAPI.Schema, table=CycleAPI.Table, condition='"WID" = :wid:', order='"StartedAt" DESC NULLS LAST', parameters={"wid": wid})

    def _cycle_runs_(self, db: PostgresDatabaseAPI, cid: str) -> list[dict]:
        return db.records(schema=RunAPI.Schema, table=RunAPI.Table, condition='"CID" = :cid:', parameters={"cid": cid})
    ...
    def _reset_(self, db: PostgresDatabaseAPI, cycle: dict, now: datetime) -> None:
        for run in self._cycle_runs_(db, cycle["UID"]):
            ...
            terminate(run["PID"])
            RunAPI.closed(run, now, Status=RunStatus.Failure.name, Duration=None, db=db).save(by="Reset")
    ...
    def _advance_(self, db: PostgresDatabaseAPI, workflow: dict, members: list[dict], edges: list, now: datetime, budget: int) -> int:
        ...
        due = launch or (kind is Kind.Scheduled and self._due_(workflow["Schedule"], cycle["StartedAt"] if cycle else None, now, workflow.get("Zone")))
        ...
        if opened:
            ...
            for tid in ready:
                if budget <= 0: break
                if not self._issue_((cycle["UID"], tid, 0)): continue
        ...
        if due:
            ...
            for tid in CoordinatorAPI.eligible(tids, edges, {}, waits=waits, tolerates=tolerates):
                ...
                if not self._timely_(rows[tid]["Schedule"], start, now, workflow.get("Zone")): continue
                if budget <= 0: break
                if not self._issue_((fresh.UID, tid, 0)): continue
    ...
    def start(self) -> None:
        ...
        while self._running_:
            try: self._tick_()
            except Exception as error: self._log_.error(lambda error=error: f"Scheduler Tick: Failed · Due to {error}")
            self._wait_()
