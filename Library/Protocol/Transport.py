...
class TransportAPI:
    ...
    def __init__(self, iid: str, create: bool = False) -> None:
        ...
        if create:
            self._ur_ = self._K32_.CreateEventW(None, False, False, f"{prefix}_ur")
            self._uc_ = self._K32_.CreateEventW(None, False, False, f"{prefix}_uc")
            self._ar_ = self._K32_.CreateEventW(None, False, False, f"{prefix}_ar")
            self._ac_ = self._K32_.CreateEventW(None, False, False, f"{prefix}_ac")
        else:
            self._ur_ = self._K32_.OpenEventW(self._EVENT_ACCESS_, False, f"{prefix}_ur")
            self._uc_ = self._K32_.OpenEventW(self._EVENT_ACCESS_, False, f"{prefix}_uc")
            self._ar_ = self._K32_.OpenEventW(self._EVENT_ACCESS_, False, f"{prefix}_ar")
            self._ac_ = self._K32_.OpenEventW(self._EVENT_ACCESS_, False, f"{prefix}_ac")
    ...
    def _wait_(self, handle) -> None:
        while True:
            ...
            result = self._K32_.WaitForSingleObject(handle, self._POLL_MS_)
            if result == 0:
                return
    ...
    def receive(self) -> bytes:
        ...
        self._K32_.SetEvent(self._uc_)
        return data

    def watchdog(self, peer_pid: int) -> None:
        handle = self._K32_.OpenProcess(self._SYNCHRONIZE_, False, peer_pid)
        if not handle:
            return
        def _watch_():
            self._K32_.WaitForSingleObject(handle, self._INFINITE_)
            self._K32_.CloseHandle(handle)
