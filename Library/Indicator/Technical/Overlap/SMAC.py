...
class SimpleMovingAverageCrossAPI(MovingAverageCrossAPI):

    Parameters = (FAST, SLOW, MODE)

    def __init__(self, name: str, fast_window: int, slow_window: int, mode: IndicatorMode) -> None:
        super().__init__(name=name, fast_window=fast_window, slow_window=slow_window, type=MovingAverageType.Simple, mode=mode)