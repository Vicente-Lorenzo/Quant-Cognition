    ...
    def run(self, args: Namespace) -> int:
        ...
        def execute() -> None:
            with PostgresDatabaseAPI(database="Quant") as db:
                ...
                rungs = self._scope_(security.Provider, category, security.Ticker, timeframe)
                trails = {}
                resolve = partial(self._parameters_, ladder, strategy, rungs, trails, folder)
                system = self._system_(args, strategy, security, timeframe, resolve)
                if system is None: return
                self._snapshot_(folder, args, getattr(system, "_parameters_", None), log, trails, rungs)
                with system:
                    system.run()
        if args.profile: profiler(execute, args.profile)()
        else: execute()
        return 0

if __name__ == "__main__":
    raise SystemExit(SystemCommandAPI().main())