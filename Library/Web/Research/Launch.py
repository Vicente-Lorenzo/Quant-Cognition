...
from Library.App.V2 import FieldAPI, ComponentID, ButtonAPI, ModalAPI, Output, Input, State, InjectionType, RefreshAPI, serverside_callback, modal_callbacks
...
class LaunchFieldsAPI:

    _STRATEGIES_ = FieldAPI.choices(StrategyType.names())
    _FITNESS_ = FieldAPI.choices(FitnessType.names())
    _SELECTION_ = FieldAPI.choices(SelectionMode.names())
    ...
    _SWAP_ = FieldAPI.choices(SwapType.names())

    SELECTION = FieldAPI(name="selection", control="select", group="objective", default=SelectionMode.Best.name, options=_SELECTION_, help="Which candidate wins inside a fold · Plateau prefers a stable neighborhood")
    PURGE = FieldAPI(name="purge", control="number", group="guard", default=0, minimum=0, help="Days trimmed from the end of each training window")
    EMBARGO = FieldAPI(name="embargo", control="number", group="guard", default=0, minimum=0, help="Days skipped after each validation window")
    TRAINING = FieldAPI(name="training", control="number", group="split", default=0, minimum=0, help="Walk-forward training window in months · 0 uses the whole period")
    ...
    MARKET = (
        ...
        FieldAPI(name="start", group="period", required=True, help="First timestamp of the simulated period"),
        FieldAPI(name="stop", group="period", required=True, help="Last timestamp of the simulated period"),
...
class LaunchAPI:
    ...
    def _launch_modal_(self) -> ModalAPI:
        return ModalAPI(id=self.LAUNCH_MODAL_ID, size="lg", centered=True, scrollable=True, open=False,
                        ...
                        body=self._form_(self._LAUNCH_) + [html.Div(id=self.LAUNCH_PREVIEW_ID, className="launch-preview")],
    ...
    def _dispatch_(self, values) -> tuple:
        ...
        requirement = FieldAPI.requirement(self._LAUNCH_, values)
        if requirement:
            self.app.notify.error(requirement, header=f"Invalid {system}")
            return dash.no_update, dash.no_update
        arguments = self._command_(values)
        if not self._trial_(lambda: self._queue_(task, arguments), "Dispatch Failed"): return dash.no_update, dash.no_update
    ...
    def launch_callbacks(fields: tuple) -> tuple:
        _open_launch_, _close_launch_ = modal_callbacks(LaunchAPI.LAUNCH_MODAL_ID, LaunchAPI.LAUNCH_BTN, LaunchAPI.LAUNCH_DISCARD_BTN)
        @serverside_callback(
