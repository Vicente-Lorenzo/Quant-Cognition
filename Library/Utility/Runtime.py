...
def tether(process: subprocess.Popen) -> Union[int, None]:
    ...
    class Extended(ctypes.Structure):
        _fields_ = [("Basic", Basic), ("IoInfo", ctypes.c_uint64 * 6), ("ProcessMemoryLimit", ctypes.c_size_t), ("JobMemoryLimit", ctypes.c_size_t),
    ...
    handle = kernel32.OpenProcess(0x0101, False, process.pid)
    bound = bool(handle) and kernel32.SetInformationJobObject(wintypes.HANDLE(job), 9, ctypes.byref(limits), ctypes.sizeof(limits)) and kernel32.AssignProcessToJobObject(wintypes.HANDLE(job), wintypes.HANDLE(handle))
    if handle: kernel32.CloseHandle(wintypes.HANDLE(handle))
    if bound: return job
    kernel32.CloseHandle(wintypes.HANDLE(job))
    return None

def release(job: Union[int, None]) -> None:
    if job is None or not is_windows(): return
    import ctypes
    from ctypes import wintypes
